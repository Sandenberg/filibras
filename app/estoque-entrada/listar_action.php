...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Entrada e')->leftJoin('e.Usuario u')
					...
					->leftJoin('e.EntradaMaterial em')->leftJoin('em.Material m')->leftJoin('e.EntradaFornecedor ef')
					->leftJoin('ef.Fornecedor f')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
            ...
            $value['fornecedor'] = isset($value['Entrada']['EntradaFornecedor']['Fornecedor']['nome_completo'])&&$value['Entrada']['EntradaFornecedor']['Fornecedor']['nome_completo']!=''?$value['Entrada']['EntradaFornecedor']['Fornecedor']['nome_completo']:"-";
