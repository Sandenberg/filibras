...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Problema p')
					->leftJoin('p.Servico s')->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Problema p')
					->leftJoin('p.Servico s')->where($where)->offset($_GET['iDisplayStart'])
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			// Status
			$value['status'] = $value['status']==1?'Solucionado':'Em aberto';
