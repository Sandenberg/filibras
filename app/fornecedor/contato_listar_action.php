...
try {
	...
	// Adição do ID da mensagem
	$where .= ' AND fornecedor_id = '.$_GET['fornecedor_id'];
	
	// Buscas (Geral e Paginada)
	$retAll		= 	Doctrine_Query::create()->select()->from('FornecedorResponsavel')->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('FornecedorResponsavel')->where($where)
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			// Telefone
			$value['telefone'] = Util::mask('(##)####-####', $value['telefone']);
