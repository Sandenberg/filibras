...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Fornecedor')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Fornecedor')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
            ...
            $value['telefone'] = strlen($value['telefone_principal'])==10?Util::mask('(##)####-####', $value['telefone_principal']):Util::mask('(##)#####-####', $value['telefone_principal']);  
                        // $value['telefone'] = Util::mask('(##)####-####', $value['telefone_principal']);  
                        
            $retFornecedoResp = Doctrine_Query::create()->select()->from('FornecedorResponsavel')
									->where("fornecedor_id = ".$value['id'])->execute();
			...
			$value['documento'] = $value['tipo_pessoa']==1?Util::mask('##.###.###/####-##', $value['cnpj']):Util::mask('###.###.###-##', $value['cpf']);
