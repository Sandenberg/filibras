...
try {
	...
	$retSituacao =	Doctrine_Query::create()->select()->from('EquipamentoSituacao')
					->where('dns LIKE "'.$_POST['dns'].'" AND id <> "'.$_POST['id'].'"')
					->execute();
	
	if ($retSituacao->count() > 0){
		...
		// Update
		$objSituacao			= Doctrine_Core::getTable('EquipamentoSituacao')->find($_POST['id']);
		$objSituacao->nome 		= $_POST['nome'];
		$objSituacao->dns		= $_POST['dns'];
		$objSituacao->status	= $_POST['status'];
		$objSituacao->save();
