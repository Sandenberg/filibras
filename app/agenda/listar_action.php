...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('Agenda a')
					->where($where);
	
	$retLimit	= 	Doctrine_Query::create()->select()->from('Agenda a')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			// Contato
			$value['contato'] = $value['nome_contato'];
			$value['contato'] .= $value['contato']!=''&&$value['email_contato']!=''?' ('.$value['email_contato'].')':'';
            ...
            $value['telefone'] = $value['id']==1?Util::mask('####-###-###', "0".$value['telefone_principal']):$value['telefone'];  
