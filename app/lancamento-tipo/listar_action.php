...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('LancamentoTipo')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('LancamentoTipo')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			// Tipo
			$value['tipo'] = $value['tipo']==1?'Crédito':'Débito';
