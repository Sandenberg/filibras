...
try {
	...
	$where = $where==''?'id <> 1':$where.' AND id <> 1';
	
	// Buscas (Geral e Paginada)
	$retAll		= 	Doctrine_Query::create()->select()->from('Usuario u')->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('Usuario u')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			// Status
			$value['status'] = $value['status']==1?'Ativo':'Inativo';
