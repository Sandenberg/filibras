...
try {
	...
	$where .= ' AND cliente_id = '.$_GET['cliente_id'];
	
	// Buscas (Geral e Paginada)
	$retAll		= 	Doctrine_Query::create()->select()->from('ClienteResponsavel')->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('ClienteResponsavel')->where($where)
