	...
	<div class="block big">
		...
		<div class="block_cont">
			...
			<form class="form" action="<?php echo URL_ADMIN.'action/'.$_GET['model'].'/'.$_GET['action'].'/'; ?>" method="POST" id="form">
				...
				$r1 = $res->restricao==1?'checked="checked"':'';
				$r0 = $res->restricao==0?'checked="checked"':'';
				...
				<div class="form_row radioui">
					...
					<input type="radio" id="tipo_pessoa0" name="tipo_pessoa" value="0" class="validate[required]" <?php echo $t0; ?> /><label for="tipo_pessoa0">Física</label> 
					<input type="radio" id="tipo_pessoa1" name="tipo_pessoa" value="1" class="validate[required]" <?php echo $t1; ?> /><label for="tipo_pessoa1">Jurídica</label>
					<input type="hidden" id="tipo_pessoa_ex" name="tipo_pessoa_ex" value="<?php echo $res->tipo_pessoa; ?>" />
				</div>
				...
				<div class="form_row radioui">
					...
					<input type="radio" id="restricao" name="restricao" value="0"  <?php echo $r0; ?> /><label for="restricao">Não</label>
					<input type="radio" id="restricao" name="restricao" value="1"  <?php echo $r1; ?> /><label for="restricao">Sim</label>
				...
				<div class="form_row">
					<label>E-mail:</label> 
					<input type="text" name="email" id="email" class="input validate[maxSize[100],custom[email]]" style="width: 313px;" value="<?php echo strtolower($res->email); ?>" />
				</div>
				...
				<span class="fisica">
					<div class="form_row">
						<label>CPF:</label> 
						<input type="text" name="cpf" id="cpf" class="input validate[required]" style="width: 177px;" value="<?php echo $res->cpf; ?>" />
					</div>
				</span>
				...
				<span class="juridica">
					<div class="form_row">
						<label>CNPJ:</label> 
						<input type="text" name="cnpj" id="cnpj" class="input validate[required]" style="width: 156px;" value="<?php echo $res->cnpj; ?>" />
					</div>
				</span>
				...
				<div class="form_row">
					...
					<input type="text" name="telefone_principal"	id="telefone_principal" class="input" style="width: 155px;" value="<?php echo ltrim($res->telefone_principal, '0'); ?>" />
				</div>

				<div class="form_row">
					<label>Tel Alternativo:</label>
					<input type="text" name="telefone_alternativo" id="telefone_alternativo" class="input" style="width: 155px;" value="<?php echo ltrim($res->telefone_alternativo, '0'); ?>" />
				</div>
				...
				<div class="form_row">
					...
					<input type="text" name="referencia" id="referencia" class="input validate[maxSize[100]]" style="width: 140px;" value="<?php echo $res->referencia; ?>" />
				</div>
				...
				<div class="form_row">
					<label>Filial:</label>
					<select name="filial_id" id="filial_id" class="select validate[required]" style="width: 150px;">
						<option value=""></option>
						<?php 
						
						try {
						
							$resFilial = Doctrine_Query::create()->select()->from('Filial')->orderBy('id ASC')->execute();
							
							if ($resFilial->count() > 0){
								$resFilial->toArray();
								
								foreach ($resFilial as $value){
									$selected = $value['id']==$res->filial_id?' selected="selected"':'';
									echo '<option value="'.$value['id'].'"'.$selected.'>'.$value['nome'].'</option>';
								}
				...
				<?php
					$resContato = Doctrine_Core::getTable('ClienteResponsavel')->findOneBy('cliente_id', $_GET['id']);
					if(!isset($resContato->id)||$resContato->id==''){
						$resContato	= new ClienteResponsavel();
						$resContato->nome = "";
						$resContato->tipo = 0;
						$resContato->cliente_id = $_GET['id'];
						$resContato->save();
					}
				...
				<div class="form_row">
					...
					<input type="text" name="nome_contato" id="nome_contato" class="input validate[required,maxSize[60]]" style="width: 440px;" value="<?php echo $resContato->nome; ?>" />
				</div>
				
				<div class="form_row">
					<label>E-mail:</label>
					<input type="text" name="email_contato" id="email_contato" class="input validate[maxSize[100],custom[email]]" style="width: 270px;" value="<?php echo $resContato->email; ?>" />
				</div>
