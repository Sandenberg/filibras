$(document).ready(function(){
    ...
    $("#tipo_nf_id").change(function(){
        if($(this).val() != ''){
            ...
            $('#nf').addClass('validate[required]');
            // $('#lancamento_nf').parent().css('display', 'inline-block');
        }else{
            $('#nf').parent().css('display', 'none');
            $('#nf').removeClass('validate[required]');
            // $('#lancamento_nf').parent().css('display', 'none');
        }
    });

    $('#recibo').change(function () {
        if ($(this).attr("checked") && $("#tipo_nf_id").val()!='') {
            alert('NF JÁ SELECIONADA!');
        }
    });
    $("#nf").change(function(){
        $.getJSON(URL_ADMIN+"getNf.php",{nf: jQuery(this).val(), tipo_nf_id: jQuery("#tipo_nf_id").val()}, function(j){
            if(j.total > 0){
    ...
    $("#tipo_nf_id").change(function(){
        if(jQuery("#nf").val()!=''){
            $.getJSON(URL_ADMIN+"getNf.php",{nf: jQuery("#nf").val(), tipo_nf_id: jQuery("#tipo_nf_id").val()}, function(j){
                if(j.total > 0){
                    alert('NF JÁ CADASTRADA PARA "'+j.cliente+'"');
                }
            });
    ...
    $("#form").submit(function(e){
        ...
        if ($("#recibo").attr("checked") && $("#tipo_nf_id").val()!='') {
            alert('DEVE SER APENAS NF OU RECIBO!');
            return false;
        }else{
            if($("#tipo_nf_id").val()!=''&&$('#nf').val()!=''){
                $.getJSON(URL_ADMIN+"getNf.php",{nf: $('#nf').val(), tipo_nf_id: jQuery("#tipo_nf_id").val()}, function(j){
                    if(j.total > 0){
                        alert('NF JÁ CADASTRADA PARA "'+j.cliente+'"');
                        return false;
                    }else{
