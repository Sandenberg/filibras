...
try {
	...
	$where = $where==''?"tipo = 'credito'":$where." and tipo = 'credito'";
	
	
	// Buscas (Geral e Paginada)
	$retAll		= 	Doctrine_Query::create()->select()->from('Lancamento l')
					->leftJoin("l.Cliente c")->leftJoin('l.LancamentoTipo lt')
					->where($where);
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			$resx = Doctrine_Core::getTable('Beneficiario')->find($value['beneficiario_id']);
			$value['nome_beneficiario'] = isset($resx->nome)&&$resx->nome!=''?$resx->nome:'-';
			
			$value['Cliente']['nome_completo'] = isset($value['Cliente']['nome_completo'])&&$value['Cliente']['nome_completo']!=''?$value['Cliente']['nome_completo']:'-';
			...
			$value['nfrb'] = isset($value['nf'])&&$value['nf']!=''?'NF':'-';
			$value['nfrb'] = isset($value['recibo'])&&$value['recibo']=='REC'&&$value['nfrb']!='NF'?'REC':$value['nfrb'];
