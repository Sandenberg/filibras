	...
	<div class="block big"><!-- Block Begin -->
		...
		<div class="block_cont">
			...
			try {
				...
				// Seleciona os dados
				$res = Doctrine_Core::getTable('Suporte')->find($_GET['id']);
			...
			<form class="form" action="<?php echo URL_ADMIN.'action/'.$_GET['model'].'/'.$_GET['action'].'/'; ?>" method="post" id="formPerfil">
				...
				<div class="form_row radioui">
					...
					$s0 = $res->troca_peca==0?'checked="checked"':'';
					$s1 = $res->troca_peca==1?'checked="checked"':'';
					
					?>
					<input type="radio" id="troca_peca1" name="troca_peca" value="1" class="input validate[required]" <?php echo $s1; ?>><label for="troca_peca1">Sim</label>
					<input type="radio" id="troca_peca0" name="troca_peca" value="0" class="input validate[required]"  <?php echo $s0; ?>><label for="troca_peca0">Não</label>
				</div>
