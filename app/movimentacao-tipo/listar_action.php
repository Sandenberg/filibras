...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('MovimentacaoTipo')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('MovimentacaoTipo')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			// Tipo
			$value['tipo'] = $value['tipo']==1?'Entrada':'Saída';
