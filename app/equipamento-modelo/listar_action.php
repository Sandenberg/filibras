...
try {
	...
	$retAll		= 	Doctrine_Query::create()->select()->from('EquipamentoModelo em')
					->leftJoin('em.Marca m')->leftJoin('em.EquipamentoTipo et')
					->where($where);
	$retLimit	= 	Doctrine_Query::create()->select()->from('EquipamentoModelo em')
					->leftJoin('em.Marca m')->leftJoin('em.EquipamentoTipo et')
	...
	if ($retLimit->count() > 0){
		...
		foreach ($resLimit as $value){
			...
			$value['numerador'] = isset($value['procedimento'])&&$value['procedimento']!=''?"OK":"";
			
			$value['EquipamentoTipo']['nome'] = isset($value['EquipamentoTipo']['nome'])&&$value['EquipamentoTipo']['nome']!=''?$value['EquipamentoTipo']['nome']:"-";
